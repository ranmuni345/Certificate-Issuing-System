...
public class FeedbackServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if((Name.equals("")||Mail.equals("")||Message.equals(""))){
            ...
            RequestDispatcher dispatcher=request.getRequestDispatcher("FeedbackFailed.jsp");
            dispatcher.forward(request, response);
            
        }else{
            
            Beans.feedbacks f1 = new Beans.feedbacks();
            
            PrintWriter out = response.getWriter();
            DAO.FeedbackDAO rb2 = new DAO.FeedbackDAO();
        
                try {
                    rb2.input(Name,Mail,Message);
                    f1.setName(Name);
                    f1.setEmail(Mail);
                    f1.setMessage(Message);
                    

                    request.setAttribute("name", f1);
